...
def calculator(set_risk_free, set_L_failure, set_inc_x,
    ...
    tmp_pi_dict = {key: 0 for key in np.arange(0, func.x_num_Max + 1, 1)}
    tmp_pi_dict[12] = 1
    tmp_pi_dict[13] = 1
    tmp_pi_dict[14] = 1
    tmp_pi_dict[15] = 1
    ...
    tmp_pi_dict[19] = 1
    tmp_pi_dict[20] = 2
    a = cal_TC_dict_MC_dict_xi_num(TC_x_num_dict, MC_x_num_dict, 11,
                                   func.Avail_E_piPtT(tmp_pi_dict, P_transition, T_x_num_dict), lag_lambda,
                                   epsilon_lag)
...
sen_4 = 1
set_gamma_alpha, set_gamma_beta = 1.5 * sen_4 * sen_4, 20 * sen_4
...
file_name = '{}_{}_{}'.format('bench', set_CVaR_rho, Avail_target)
writer = pd.ExcelWriter('./result_last_20_2/{}_{}.xlsx'.format(file_name, epsilon_lag),
                        engine='xlsxwriter')
